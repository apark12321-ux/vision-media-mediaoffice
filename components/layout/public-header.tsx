...
export function PublicHeader() {
  return (
    <header className="border-b bg-white">
      <div className="border-b border-slate-100 bg-slate-50">
        <div className="mx-auto flex max-w-[1180px] flex-col gap-2 px-4 py-2 text-[12px] text-slate-500 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
            <span className="font-black text-slate-800">2026.06.18(목)</span>
            <span>교육산업 · 평생학습 · 에듀테크 전문 매체</span>
      ...
      <div className="mx-auto flex max-w-[1180px] flex-col gap-4 px-4 py-5 md:flex-row md:items-end md:justify-between">
        <Link href="/" className="block">
          <p className="text-[11px] font-black tracking-[0.34em] text-blue-700">EDUCATION INDUSTRY JOURNAL</p>
          <div className="mt-1 flex items-end gap-3">
            <span className="text-[42px] font-black leading-none tracking-[-0.08em] text-slate-950 md:text-[56px]">에듀저널</span>
            <span className="pb-1 text-[13px] font-black tracking-[0.25em] text-slate-400">EDU JOURNAL</span>
          </div>
          <p className="mt-2 text-[13px] font-semibold text-slate-500">매일 만나는 교육 산업·정책·기관·기술 소식</p>
        </Link>
        <form action="/search" className="flex h-10 w-full max-w-[380px] overflow-hidden border border-slate-300 bg-white">
      ...
      <nav className="border-y border-slate-950 bg-white text-slate-950">
        <div className="mx-auto flex h-12 max-w-[1180px] items-center overflow-x-auto px-4 text-[15px] font-black">
          {navItems.map(([label, href]) => (
            <Link
              key={href}
              href={href}
              className="shrink-0 border-r border-slate-200 px-4 py-3 hover:bg-slate-950 hover:text-white first:border-l"
            >
              {label}
            </Link>
          ))}
