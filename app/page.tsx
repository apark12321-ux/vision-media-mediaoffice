...
function BannerAd({ src, alt, className = '' }: { src: string; alt: string; className?: string }) {
  ...
  return <aside className={`overflow-hidden border border-slate-200 bg-white shadow-sm ${className}`} aria-label={alt}><div className="flex h-full flex-col justify-center bg-slate-950 px-4 py-3 text-white"><p className="text-[10px] font-black tracking-[0.22em] text-amber-400">SPONSORED</p><p className="mt-1 text-sm font-black opacity-80">{brand}</p><h3 className={`${isWide ? 'text-2xl' : 'text-lg'} mt-1 font-black tracking-[-0.04em]`}>{copy}</h3><span className="mt-2 inline-flex w-fit rounded-full bg-amber-500 px-3 py-1.5 text-[11px] font-black text-white">상담 신청</span></div></aside>;
}
