...
function BannerAd({ src, alt, className = '' }: { src: string; alt: string; className?: string }) {
  return <aside className={`overflow-hidden border border-slate-200 bg-white shadow-sm ${className}`} aria-label={alt}><img src={src} alt={alt} className="h-full w-full object-contain" loading="lazy" /></aside>;
}
