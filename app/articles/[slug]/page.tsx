...
function ArticleImage({ article }: { article: Article }) {
  ...
  return (
    <figure className="mt-7">
      ...
      <figcaption className="mt-2 border-l-2 border-slate-300 pl-3 text-[13px] leading-6 text-slate-500 md:text-[13px]">
...
function LeadSummary({ article }: { article: Article }) {
  return (
    <p className="mt-7 text-[20px] font-semibold leading-9 text-slate-700 md:text-[21px] md:leading-10">
      {article.summary}
    </p>
  );
}
...
function BodyContent({ article }: { article: Article }) {
  ...
  return (
    <div className="mt-8 space-y-5 text-[18px] leading-[2.0] text-slate-800 md:text-[19px] md:leading-[2.05]">
      ...
      <div className="mx-auto grid max-w-[1120px] gap-8 px-4 py-7 md:py-9 lg:grid-cols-[minmax(0,760px)_300px]">
        <article>
          ...
          <header className="mt-7 border-b border-slate-200 pb-6 md:mt-8">
            <p className="text-[14px] font-black text-brand-blue">{article.categories?.name ?? '교육뉴스'} · 일반기사</p>
            <h1 className="mt-3 text-[35px] font-black leading-[1.22] tracking-[-0.055em] md:text-[46px] md:leading-tight">{article.title}</h1>
            <p className="mt-4 text-[19px] font-semibold leading-9 text-slate-600 md:text-[20px] md:leading-9">{article.subtitle || article.summary}</p>
            <div className="mt-6 flex flex-wrap gap-x-5 gap-y-2 border-t border-slate-200 pt-4 text-[14px] leading-6 text-slate-500">
          ...
          <p className="mt-9 border-y border-slate-200 py-5 text-[17px] leading-9 text-slate-700 md:text-[18px]">
            에듀저널은 교육 현장에서 확인되는 제도 변화와 학습자 선택 기준을 계속 점검하고, 관련 기관의 운영 정보와 실제 활용 사례를 함께 보도할 예정이다.
          </p>

          {tags.length ? (
            <section className="mt-6 flex flex-wrap gap-2">
              {tags.map((tag) => <span key={tag} className="rounded-full border border-slate-200 px-3 py-1.5 text-[14px] font-bold text-slate-600">#{tag}</span>)}
            </section>
          ) : null}

          <section className="mt-10 rounded-sm bg-slate-50 p-5">
            <p className="text-[16px] font-black text-slate-800">제보·정정 안내</p>
            <p className="mt-2 text-[16px] leading-7 text-slate-600">교육 현장 제보, 사실 정정 요청, 보도자료 접수는 기사제보 페이지를 통해 보낼 수 있습니다.</p>
            <Link href="/report" className="mt-3 inline-flex text-[15px] font-black text-brand-blue">기사제보 바로가기 ›</Link>
          </section>
